...
//заполним нашу таблицу данными из массива
for(let i in array){ //цикл пробегается по каждому элементу массива
    var table = document.getElementById("main-table"); //привязываем table к нашей таблице
    let newrow = table.insertRow(-1); // создаем новый ряд для каждого элемента масива
    newrow.insertCell(-1).innerHTML = array[i].name.lastName;  //добfвляем новую ячейку в конец ряда и заполняем данными из обьекта
    newrow.insertCell(-1).innerHTML = array[i].name.firstName;
    newrow.insertCell(-1).innerHTML = array[i].about; 
    newrow.insertCell(-1).style.backgroundColor = array[i].eyeColor;//колонке цвет глаз устанавливаем соотв. фон
}

//создаем радом таблицу кнопок "редактировать"
var rowCounter=table.getElementsByTagName("TR"); //массив из всех рядов таблицы
for(let i=1;i<rowCounter.length;i++){ 
    var chngtable = document.getElementById("change-table"); 
    let newrow = chngtable.insertRow(-1); 
    let newCell = newrow.insertCell(-1);
    newCell.classList.add("trheight");
    newCell.innerHTML = "<button> &#9998 </button>"; 
    newCell.setAttribute("onclick","change("+i+")");// вызывает при нажатии функцию редактирования соответствующей строки
    newCell.setAttribute("id","chngid"+i); //каждой ячейке присваиваем соотв. ид
}

var tdtags = table.getElementsByTagName("TD");//возвращает массив всех <td> в таблице 
var chngRowCounter=chngtable.getElementsByTagName("TR"); //массив из всех рядов таблицы с кнопками редактировать
for (let i=0; i<tdtags.length;i++){  
    tdtags[i].setAttribute("class","tdheight");
}


//добавим обрезку содержимого колонки описание
for (let i = 6; i < tdtags.length; i+=4) { //проходим по массиву все тегов <td>, выбирая все разделы  about
    tdtags[i].setAttribute("class","mystyle");// нужным <td> добавляем класс mystyle, далее с помощью CSS обрезаем содержимое и добавляем многоточие
}

var j = 0;
for (let i = 7; i < tdtags.length; i+=4) { 
    
    tdtags[i].setAttribute("colorEye",array[j].eyeColor); //ячейкам  цвет глаз устанавливаем соотв. атрибут
    j++;
}

var countpage=1; //счетчик отображаемый страниц
var screen=document.getElementById("screen");
screen.innerHTML=((countpage-1)*10+1)+"-"+countpage*10; // отображаем номера текущих строк

//отображаем по умолчанию с 1 по 10 страницы
    for (let i=11;i<=50;i++){
        rowCounter[i].hidden=true;
        chngRowCounter[i].hidden=true;
    }
    for (let i=1;i<=10;i++){
        rowCounter[i].hidden=false;
        chngRowCounter[i].hidden=false;
    }

function nextpage(){// перелистываем вперед
        countpage++;//счетчик++
        if(countpage>5){
            countpage=1;//если выходим за рамки отображаем сначала
        }
        for (let i=1;i<=50;i++){ // прячем все элементы
            rowCounter[i].hidden=true;
            chngRowCounter[i].hidden=true;
        }
        for (let i=1+(countpage-1)*10;i<=countpage*10;i++){ // отображаем нужные элементы
            rowCounter[i].hidden=false;
            chngRowCounter[i].hidden=false;
        }
        screen.innerHTML=((countpage-1)*10+1)+"-"+countpage*10;  // отображаем номера текущих строк
...
function backpage(){  //тоже самое но назад
    countpage--;
    if(countpage<1){
        countpage=5;
    }
    ...
    for (let i=1+(countpage-1)*10;i<=countpage*10;i++){
        rowCounter[i].hidden=false;
        chngRowCounter[i].hidden=false;
    }
    screen.innerHTML=((countpage-1)*10+1)+"-"+countpage*10;
}

var direct=1;//переменная отвечает за направление сортировки
function sortTable(n) {//функция отвечает за сортировку соотвествующего столбца таблицы (использую метод пузырька)
    //скрипт для отображения треугольничков с направление сортировки
    for (let i=0;i<=3;i++){ //очищаем все имеющиеся треугольнички
        table.getElementsByTagName("td")[i].classList.remove("down");
        table.getElementsByTagName("td")[i].classList.remove("up");
    }
    if(direct==1){//в зависимости от направления добавляем посредством CSS тругольничек нужной колонке
        table.getElementsByTagName("td")[n].classList.add("up");
        table.getElementsByTagName("td")[n].classList.remove("down");
    }   else{
        table.getElementsByTagName("td")[n].classList.add("down");
        table.getElementsByTagName("td")[n].classList.remove("up");
    }
    let rows = table.getElementsByTagName("TR");//создаем массив rows включающий в себя ряды нашей таблицы
    var counter=true; //переменная "выключатель" сортировки
    while(counter){ //пока выключатель работает..
        var y=0;//переменная счетчик
        for(let i=1+(countpage-1)*10;i<=countpage*10-1;i++){//прогоняем цикл по массиву ОТОБРАЖАЕМЫХ рядов
        if (n!==3){//отедльное условия для колонки с цветом
            var cells1=rows[i].getElementsByTagName("TD")[n];//значение текущей ячейки
            cells1 = cells1.innerHTML;//значение текущей ячейки
            var cells2=rows[i+1].getElementsByTagName("TD")[n];//значение следующей ячейки
            cells2 = cells2.innerHTML;//значение текущей ячейки 
        } else {
            var cells1=rows[i].getElementsByTagName("TD")[n];
            cells1 = cells1.getAttribute("colorEye");//помещаем в переменную значение атрибута, для дальнейшего сравнивания
            var cells2=rows[i+1].getElementsByTagName("TD")[n];
            cells2 = cells2.getAttribute("colorEye");
        }
            switch (direct){//в зависимости от направления сортировки...
                case 1: if (cells1>cells2){//если значение текущей ячейки больше значения следующей
                        
                        y++;//добавляем счетчик
                        rows[0].parentNode.insertBefore(rows[i+1], rows[i]);//меняем местами текщий и следующий ряд (было сложно и одновременно интересно разобраться с работой данных методов)
                    }
                    break;
                case 0: if (cells1<cells2){//тоже самое, но наоборот
                        y++;
        ...
        if (y==0){//если цикл прошелся по всему массиву и не разу не выполнил перестановку(счетчик = 0)
            switch(direct) {
                case 1: direct=0;//меняем направление сортировки для следующего вызова  
                break;
                case 0: direct=1;
                break;
            }
            counter = false;//выключаем наш "выключатель"
        }
    }
}
...
function change(n){
    ...
    rows[n].classList.add("big-border");
    let cells = rows[n].getElementsByTagName("TD");//создаем массив ячеек выброной строки
    let firstName=cells[0]; // каждую ячейку по порядку привязываем к соотв.значению
    let lastName=cells[1];
    let about=cells[2];
    let eyeColor=cells[3];
  
    chngtable.hidden=true;//на время редактирования прячем таблицу с кнопками
    form.hidden=false;//отображаем форму редактирования
    var changeFirstName = document.getElementById("name-label");//привязываем переменную к соотв. полю ввода
        changeFirstName.value =firstName.innerHTML;//по умолчанию заполняем поле ввода данными из ячейки
        
    var changeLastName = document.getElementById("last-name-label");
        changeLastName.value =lastName.innerHTML;
        
    var changeAbout = document.getElementById("textComent");
        changeAbout.value = about.innerHTML;
        
    var changeEyeColor = document.getElementById("eyeDrop");
        changeEyeColor.value =eyeColor.getAttribute("colorEye");
        
    var okey = document.getElementById("submit");//кнопка ввода 
        okey.onclick = () => {//при нажатии

            firstName.innerHTML=changeFirstName.value;//присваиваем каждой ячейке значение введенное в поле ввода
            lastName.innerHTML=changeLastName.value;
            about.innerHTML=changeAbout.value;
            eyeColor.setAttribute("colorEye",changeEyeColor.value);
            eyeColor.style.backgroundColor=eyeColor.getAttribute("colorEye");//меняем цвет ячейки на выбранный
            chngtable.hidden=false; //отображаем кнопки редактирования
            form.hidden=true; //прячем форму 
            rows[n].classList.remove("big-border");
    };
    var cancel = document.getElementById("cancel"); //кнопка отмена
        cancel.onclick = () => {//ничего не меняем
            chngtable.hidden=false; //отображаем кнопки редактирования
            form.hidden=true; //прячем форму 
            rows[n].classList.remove("big-border");
        }
...
//фунция скрыть/показать колонку
var hid={"0":0,"1":0,"2":0,"3":0};//по умлочанию выставляем для каждого номера колонки выключатель

function hide(n){
    switch(hid[n]){//в зависимости от переключателя
        case 0: 
            hid[n]=1;//переключаем для следюиспользования
            for(let i=n;i<tdtags.length;i+=4){ //проходимся по всем тегам TD соотв. колонки
                tdtags[i].hidden=true; //прячем все
                if (n==2){ // если обрабатывается колонка "описание",
                    tdtags[i].classList.remove("mystyle");//убираем стилизацию для корректного отображения
                }
            }
            break; 
        case 1:  //аналогично но наоборот
            hid[n]=0;
            for(let i=n;i<tdtags.length;i+=4){
                tdtags[i].hidden=false; 
                if ((n==2) && (i!=2)){
                    console.log(i);
                    tdtags[i].classList.add("mystyle");
                }
            }
            break; 
    }
